...
class FreqObserver(threading.Thread):
    def __init__(self, freqs):
        ...
        self._freqs = freqs
        self._freqs_0 = sorted(iter(self._freqs.items()), key=operator.itemgetter(1), reverse=True)[:25]
        self.start()
    ...
    def _update_view(self):
        lock.acquire()
        freqs_1 = sorted(iter(self._freqs.items()), key=operator.itemgetter(1), reverse=True)[:25]
        lock.release()
